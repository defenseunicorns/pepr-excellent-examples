...
describe("warnings.ts", () => {
  ...
  describe("warnings with approval", () => {
    ...
    it(
      "approves resource with warnings",
      async () => {
        // Delete the resource first to ensure we get a fresh creation with warnings
        execSync(`kubectl delete -f ${scenarioFile} --ignore-not-found`, {
          encoding: "utf-8",
        });
        ...
        expect(result.stderr).toContain(
          "Warning: The 'deprecated-field' is being used",
        );
        expect(result.stderr).toContain(
          "Warning: Best practice is to include an 'app' label",
        );
        expect(result.stderr).toContain(
          "Warning: Large number of configuration items detected",
        );
  ...
  describe("multiple warnings in approval", () => {
    ...
    it(
      "approves resource with multiple warnings",
      async () => {
        // Delete the resource first to ensure we get a fresh creation with warnings
        execSync(`kubectl delete -f ${scenarioFile} --ignore-not-found`, {
          encoding: "utf-8",
        ...
        expect(result.stderr).toContain(
          "Warning: The value 'insecure' for 'setting2' is not recommended",
        );
