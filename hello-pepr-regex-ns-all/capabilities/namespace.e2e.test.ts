...
describe("namespace.ts", () => {
  ...
  describe("unspecified module namespaces", () => {
    ...
    it("handles: default regex namespace resources", async () => {
      const cm = await K8s(kind.ConfigMap).InNamespace("default").Get("cm-def")
      expect(cm.metadata?.annotations).toEqual(expect.objectContaining({
        def: "seen",
        non: "seen"
      }))
    }, secs(10))

    it("handles: non-default regex namespace resources", async () => {
      const cm = await K8s(kind.ConfigMap).InNamespace("hello-pepr-ns-all").Get("cm-ns")
      expect(cm.metadata?.annotations).toEqual(expect.objectContaining({
        ns: "seen",
        non: "seen"
      }))
