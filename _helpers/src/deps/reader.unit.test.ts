...
const resolved = obj => (_path => Promise.resolve(buffered(obj))) as typeof fs.readFile;
...
describe("reader()", () => {
  ...
  it("returns a found-dependency set", async () => {
    ...
    access.mockImplementation(_p => Promise.resolve());
    nearestAncestor.mockImplementation((_f, _d) => me);
