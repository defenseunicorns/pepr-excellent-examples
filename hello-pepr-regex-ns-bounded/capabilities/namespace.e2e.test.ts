...
describe("namespace.ts", () => {
  ...
  describe("module-owned namespaces", () => {
    ...
    it(
      "handles: in-scope resources with filtered namespaces",
      async () => {
        const one = await K8s(kind.ConfigMap)
          .InNamespace("hello-pepr-ns-alpha")
          .Get("one");
        expect(one.metadata?.annotations).toEqual(
          expect.objectContaining({ a: "alpha" }),
        );

        const two = await K8s(kind.ConfigMap)
          .InNamespace("hello-pepr-ns-bravo")
          .Get("two");
        expect(two.metadata?.annotations).toEqual(
          expect.objectContaining({ b: "bravo" }),
        );
      },
      secs(10),
    );
    ...
    it("handles: in-scope resources without filtered namespaces", async () => {
      const one = await K8s(kind.ConfigMap)
        ...
        .Get("one");
      expect(one.metadata?.annotations).toEqual(
        expect.objectContaining({ c: "charlie" }),
      );
    ...
    it(
      "ignores: out-of-scope resource without filtered namespaces",
      async () => {
        const three = await K8s(kind.ConfigMap)
          .InNamespace("default")
          .Get("three");
