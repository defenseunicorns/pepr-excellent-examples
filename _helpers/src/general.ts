...
// Vitest runs test files in parallel so we can't guarantee that test capabilities
...
export async function waitLock(file: string, unique: string) {
  const lock = async () => {
    ...
    try {
      fileHandle = await fsP.open(file, "wx");
    } catch (e) {
      if (e.code === "EEXIST") {
        return false;
      } else {
...
export function nearestAncestor(filename: string, fromPath: string): string {
  ...
  const searchPaths = [];
  parts.forEach((_, idx) =>
    searchPaths.push(starp.slice(idx, parts.length).reverse().join(path.sep)),
  );
