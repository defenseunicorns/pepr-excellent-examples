...
describe("differ()", () => {
  ...
  it("update non-pinned deps from remote lookup", async () => {
    ...
    expect(result.updates).toEqual([{ name: "@just/doit", from: "1.2.2", to: "1.2.3" }]);
  });
  ...
  it("updates non-pinned deps & keeps pre-existing range specs", async () => {
    ...
    versions.mockImplementationOnce(() =>
      Promise.resolve({
        name: "@just/dothis",
        "dist-tags": {
          latest: "1.2.4",
  ...
  describe("@types/node", () => {
    it("update based on version of 'typescript' dep", async () => {
      ...
      expect(result.updates).toEqual([{ name: "@types/node", from: "1.2.3", to: "22.7.3" }]);
    });
  });
});
