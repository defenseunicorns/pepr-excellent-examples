...
describe("clean()", () => {
  ...
  beforeAll(async () => {
    ...
    trc = {
      kubeConfig,
      labelKey: vi.fn(() => "test-transient/capability-name"),
  ...
  it(
    "removes resources with TestRunCfg-defined label",
    async () => {
      const configMap = {
        apiVersion: "v1",
        kind: "ConfigMap",
  ...
  it(
    "removes CRD & CRs with TestRunCfg-defined label",
    async () => {
      const crd = {
