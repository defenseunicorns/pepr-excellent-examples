...
describe("ClusterPolicyReport", () => {
  beforeAll(async () => {
    ...
    await timed("load UDS Exemption CRD", async () => {
      await K8s(kind.CustomResourceDefinition).Apply(UDSExemptionCRD);
    });
  ...
  it(
    "is created when UDS Exemption exists",
    async () => {
  ...
  it(
    "is deleted when UDS Exemptions are gone",
    async () => {
      await K8s(Exemption)
        .InNamespace("pexex-clusterpolicyreport")
        .Delete("allow-naughtiness");
      await untilTrue(() =>
        gone(ClusterPolicyReport, { metadata: { name: "pepr-report" } }),
      );
    },
    secs(30),
  );
  ...
  it(
    "has a result for each UDS Exemption policy",
    async () => {
      const cpr = await K8s(ClusterPolicyReport).Get("pepr-report");
