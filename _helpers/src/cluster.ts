import { GenericClass, K8s, kind, KubernetesObject, RegisterKind } from "kubernetes-fluent-client";
...
async function retry<T, U>(
  action: AsyncFunc<T>,
  reactions: Record<string, AsyncFunc<U>>,
  retries: number = 3,
): Promise<T> {
  try {
    return await action();
...
export async function up(name: string = "pexex-helpers-cluster"): Promise<string> {
  ...
  await Promise.all(
    kinds.map(
      async k =>
        await retry(async () => await K8s(k).Get(), {
          "400": async () => {
            await sleep(5);
          },
          "404": noop,
...
export async function clean(trc: TestRunCfg): Promise<void> {
  ...
  try {
    ...
    tbds.forEach(
      async ([k, o]) =>
        await retry(async () => await K8s(k).Delete(o), {
          "429": async (err: KfcErr) => {
            await sleep(err.data.details ? err.data.details.retryAfterSeconds : 1);
          },
        }),
