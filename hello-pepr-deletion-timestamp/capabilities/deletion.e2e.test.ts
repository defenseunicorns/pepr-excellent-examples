...
describe("deletion.ts", () => {
  ...
  describe("tests WithDeletionTimestamp across admission and watch bindings", () => {
    let logz: string[]
     ...
     beforeAll(async () => {
      ...
      await timed(`load: ${file}`, async () => {
        let [ ns1, ns2, ns1Admission, n1Watch, ns2Admission, ns2Watch, ns2Delete ] = await trc.load(file)

        await fullCreate([ns1, ns2, ns1Admission, n1Watch, ns2Admission, ns2Watch, ns2Delete])  
        await K8s(kind.Pod).InNamespace("hello-pepr-deletion-timestamp2").Delete("ns2-delete")
    ...
    it("tests that the filters match for watch and admission on update event", () => {
    ...
    it("tests that the filters do not match for watch and admission on update even with no deletionTimestamp", () => {
      const results = logz.filter(l => l.includes("DTS:"))
      let wants = [
    ...
    it("tests that the filters match for watch on delete even with deletionTimestamp", () => {
      const results = logz.filter(l => l.includes("DTS:"))
      let wants = [
        "DTS: Saw a pod ns2-delete.",
      ]
      wants.forEach((wanted) => {
        expect(JSON.stringify(results)).toContain(wanted)
      })
